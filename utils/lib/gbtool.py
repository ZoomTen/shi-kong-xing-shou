...
def get_symbol_or_undefined(symbol_table, address, returns=None):
	'''
	arguments:
	...
	-	"returns" - Optional. Defaults to None.
		What to return if it doesn't find anything. This can be
		a function that takes a single argument. The function's
		input will be a tuple of (BANK, ADDRESS) which is what
		`offset2addr` returns.

	returns:
	-	the corresponding symbol or None
	'''
	if address in symbol_table:
		return symbol_table[address]
	else:
		if callable(returns):
			return returns(offset2addr(address))
		else:
			return returns
...
def get_symbol(symbol_table, address):
	...
	'''
	return get_symbol_or_undefined(
		symbol_table, address,
		returns=lambda x:('unk_%03x_%04x' % x)
	)
