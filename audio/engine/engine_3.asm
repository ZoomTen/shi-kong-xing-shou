SoundEngine3_Load:
	or a
	jp z, SoundEngine3_ResetSoundRegisters
	cp $a
	jp z, SoundEngine3_Fade
	push bc
	push de
	ld [wCurrentSongID], a
...
; Calculate song header location.
	push af
	add a
	add LOW(SoundEngine3_HeaderPointers)
	ld l, a
	ld a, HIGH(SoundEngine3_HeaderPointers)
	adc 0
	ld h, a
	pop af
	ld a, [hli]
...
; Set default instrument parameters?
	...
	add hl, de
	ld a, LOW(SoundEngine3_VibratoTables.vibrato1)
	ld [hli], a
	ld a, HIGH(SoundEngine3_VibratoTables.vibrato1)
	ld [hl], a
	pop bc
	ld a, [wSoundChannelIndex]
	push af
	add LOW(SoundEngine3_ChannelMaskTable2)
	ld l, a
	ld a, HIGH(SoundEngine3_ChannelMaskTable2)
	adc 0
	ld h, a
	pop af
...
.loop1
	...
	or a
	jr z, .skip
	call SoundEngine3_UpdateChannels
	call Func_01d_4a51
.skip
	ld hl, CHANNEL_STRUCT_LENGTH
	add hl, de
...
SoundEngine3_UpdateChannels:
	ld hl, CHANNEL_FIELD08
	add hl, de
	inc [hl]
	dec hl
	dec [hl]
	call z, SoundEngine3_LoadNote
...
SoundEngine3_ReadMusic:
	ld a, [bc]
	cp $d0
	jr c, .notes_and_rests
	call SoundEngine3_CommandProcessor
	jr SoundEngine3_ReadMusic
...
.is_note
	...
	push af
	add a
	add LOW(SoundEngine3_NoteFrequencies)
	ld c, a
	ld a, HIGH(SoundEngine3_NoteFrequencies)
	adc 0
	ld b, a
	pop af
...
.note_commit
	...
	ld a, [bc]
	and $f
	call SoundEngine3_SetLengthCounter
	ld hl, CHANNEL_LENGTH_COUNTER
	add hl, de
	ld [hl], a
	call SoundEngine3_WriteToPlayhead
	ld hl, CHANNEL_ENV_MODE
	add hl, de
	bit 7, [hl]
...
; Point channel 7 at the drum sound-effect table and mark it active.
	ld a, LOW(unk_01d_4e6a)
	ld [wSound1Channel7Playhead], a
	ld a, HIGH(unk_01d_4e6a)
...
SoundEngine3_SetLengthCounter:
	ld hl, CHANNEL_SPEED
	add hl, de
	or a
	jr z, SoundEngine3_LowNibbleOnlySwapped

SoundEngine3_AtimesHL:
	cp [hl]
	jr nc, .less_than_hl
...
SoundEngine3_LowNibbleOnlySwapped:
	ld a, [hl]
	swap a
	and $f0
	ret
...
; set type (y)
	...
	push af
	add a
	add LOW(SoundEngine3_VibratoTables)
	ld c, a
	ld a, HIGH(SoundEngine3_VibratoTables)
	adc 0
	ld b, a
	pop af
...
.cmd_ee_ptr_setup
	...
	push af
	add a
	add LOW(SoundEngine3_InstrumentPointers)
	ld c, a
	ld a, HIGH(SoundEngine3_InstrumentPointers)
	adc 0
	ld b, a
	pop af
	ld hl, CHANNEL_INSTRUMENT_POINTER
...
.cmd_ff
	ld a, [wSoundCurChannel]
	push af
	add LOW(SoundEngine3_ChannelMaskTable1)
	ld c, a
	ld a, HIGH(SoundEngine3_ChannelMaskTable1)
	adc 0
	ld b, a
	pop af
	ld a, [bc]
...
SoundEngine3_ChannelMaskTable1:
	db $fe  ; ch1
	db $fd  ; ch2
	db $fb  ; ch3
	db $f7  ; ch4
...
SoundEngine3_ChannelMaskTable2:
	db $01  ; ch1
	db $02  ; ch2
	db $04  ; ch3
	db $08  ; ch4
...
; apply Ch1 duty cycle
	...
	inc de
	inc c
	call SoundEngine3_UpdateLastData

; apply Ch1 envelope
	ld a, b
	call SoundEngine3_DoUpdateEnvelopes

; apply Ch1 note frequency
	ld a, b
	call SoundEngine3_CheckNeedUpdateLastData
	call SoundEngine3_DoUpdateFrequency
...
; find waveform data
	...
	push af
	add a
	add LOW(SoundEngine3_Waveforms)
	ld l, a
	ld a, HIGH(SoundEngine3_Waveforms)
	adc 0
	ld h, a
...
SoundEngine3_DoUpdateEnvelopes:
	or a
	jr z, .write_reg
	ld a, [de]
	cp [hl]
	jr z, SoundEngine3_SkipUpdatingLastData
...
SoundEngine3_CheckNeedUpdateLastData:
	or a
	jr z, SoundEngine3_DoUpdateLastData

SoundEngine3_UpdateLastData:
	ld a, [de]
	cp [hl]
	jr z, SoundEngine3_SkipUpdatingLastData

SoundEngine3_DoUpdateLastData:
	ld a, [hl]
	ld [de], a
	ldh [c], a

SoundEngine3_SkipUpdatingLastData:
	inc hl
	inc de
	inc c
	ret

SoundEngine3_DoUpdateFrequency:
	ld a, [de]
	bit 7, a
	jr nz, .write_reg
	cp [hl]
...
SoundEngine3_ResetSoundRegisters:
	call .DoReset

.DoReset:
	push bc
	push de
	call SoundEngine3_TurnOffChannels
	ld a, $77
	ld [wSoundGlobalStereo], a
	jr SoundEngine3_ResetEngineVariables

SoundEngine3_Init:
	push bc
	push de
	call SoundEngine3_ResetHWVolumes
	ld a, $77
	ldh [rNR50], a
	ld [wSoundGlobalStereo], a

SoundEngine3_ResetEngineVariables:
	ld hl, wSound1Channels
	ld de, CHANNEL_STRUCT_LENGTH
	ld a, 0
	ld [hl], a
...
SoundEngine3_ResetHWVolumes:
	xor a
	ldh [rNR50], a
	ld a, $80
	ldh [rNR52], a

SoundEngine3_TurnOffChannels:
; reset sweep
	ld a, 8
	ldh [rNR10], a
	ld [wCh1DataCurrentSweep], a
...
SoundEngine3_CommandF4:
	...
	cp $11
	jr z, .ptr_setup
	jp SoundEngine3_CommandProcessor.cmd_f5

.ptr_setup:
	ld a, [hl]
	jp SoundEngine3_CommandProcessor.cmd_ee_ptr_setup
